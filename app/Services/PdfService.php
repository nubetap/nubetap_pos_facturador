...
class PdfService
{
    ...
    protected function setPaperFormat(Dompdf $pdf, string $format): void
    {
        ...
        // Normalizar formato para comparación case-insensitive
        $formatLower = strtolower($format);

        if ($formatLower === 'a4' || $formatLower === 'a5') {
            // Para formatos estándar, usar el nombre del formato directamente
            $pdf->setPaper(strtoupper($formatLower), 'portrait');
        } else {
            // Para formatos personalizados (80mm, 50mm, ticket)
    ...
    public function generateRetentionPdf($retention, string $format = 'A4'): string
    {
        $data = $this->prepareRetentionData($retention);
        $data['format'] = $format;

        $template = $this->getTemplate('retention', $format);
        $html = View::make($template, $data)->render();
    ...
    protected function prepareRetentionData($retention): array
    {
        // Preparar datos de la empresa (pasar como objeto para compatibilidad con componentes)
        $company = $retention->company;
        $detalles = $this->safeJsonDecode($retention->detalles);

        // Calcular totales
        $totalDocumentos = 0;
        $totalRetenido = 0;
        foreach ($detalles as $detalle) {
            $totalDocumentos += $detalle['imp_total'] ?? 0;
            $totalRetenido += $detalle['imp_retenido'] ?? 0;
        }

        return [
            'document' => $retention,
            'retention' => $retention,
            'company' => $company, // Pasar objeto completo en lugar de array
            'tipo_documento_nombre' => 'COMPROBANTE DE RETENCIÓN',
            'numero' => $retention->numero_completo ?? ($retention->serie . '-' . $retention->correlativo),
            'fecha_emision' => $retention->fecha_emision->format('d/m/Y'),
            'moneda' => $retention->moneda,
            'hash' => $retention->hash_cdr,
            'qr_code' => null,
            'detalles' => $detalles,
            'totales' => [
                'total_documentos' => $totalDocumentos,
                'total_retenido' => $retention->imp_retenido ?? $totalRetenido,
                'total_pagado' => $retention->imp_pagado ?? 0,
                'regimen' => $retention->regimen,
                'tasa' => $retention->tasa,
            ],
            'total_en_letras' => $this->numeroALetras($retention->imp_retenido ?? $totalRetenido, $retention->moneda),
        ];
    }
}