...
class GreenterService
{
    ...
    public function __construct(Company $company, StorageService $storageService)
    {
    ...
    protected function initializeSee(): See
    {
        ...
        try {
            $certificadoContent = $this->storageService->getCertificateContent($this->company->ruc);

            if ($certificadoContent === null) {
                throw CertificateException::notFound(
                    "Certificado no encontrado para RUC: " . $this->company->ruc
                );
            }

            // Validar estructura PEM
            if (!$this->isValidPemStructure($certificadoContent)) {
                throw CertificateException::invalid();
            }
    ...
    protected function initializeSeeApi()
    {
        ...
        try {
            ...
            if ($certificadoContent === null) {
                throw new Exception(
                    "Certificado no encontrado para GRE - RUC: " . $this->company->ruc
                );
            }
    ...
    public function createInvoice(array $invoiceData): GreenterInvoice
    {
        ...
        if ($tipoOperacion === '0200') {
            ...
            \Log::info('=== VALORES ENVIADOS A GREENTER ===', [
                'mto_oper_gravadas' => $invoiceData['mto_oper_gravadas'],
                'mto_igv' => $invoiceData['mto_igv'],
                'valor_venta' => $invoiceData['valor_venta'],
                'sub_total' => $invoiceData['sub_total'],
                'mto_imp_venta' => $invoiceData['mto_imp_venta'],
                'tiene_descuentos' => isset($invoiceData['descuentos']),
                'tiene_anticipos' => isset($invoiceData['anticipos'])
            ]);
            ...
            if (($invoiceData['mto_isc'] ?? 0) > 0 && isset($invoiceData['detalles'])) {
                $totalBaseIsc = array_reduce($invoiceData['detalles'], function($carry, $detalle) {
                    return $carry + ($detalle['mto_base_isc'] ?? 0);
                }, 0);

                if ($totalBaseIsc > 0) {
                    $invoice->setMtoBaseIsc($totalBaseIsc);
                }
            }
        }
        ...
        if (isset($invoiceData['descuentos']) && is_array($invoiceData['descuentos']) && count($invoiceData['descuentos']) > 0) {
            $descuentosGlobales = [];
            foreach ($invoiceData['descuentos'] as $desc) {
                $descuento = new \Greenter\Model\Sale\Charge();
                $descuento->setCodTipo($desc['cod_tipo'] ?? '03')
                          ->setMontoBase($desc['monto_base'] ?? 0)
                          ->setFactor($desc['factor'] ?? 0)
                          ->setMonto($desc['monto'] ?? 0);
                $descuentosGlobales[] = $descuento;
            }
            $invoice->setDescuentos($descuentosGlobales);
        }

        // Anticipos (según ejemplo de Greenter)
        if (isset($invoiceData['anticipos']) && is_array($invoiceData['anticipos']) && count($invoiceData['anticipos']) > 0) {
            $anticipos = [];
            $totalAnticipos = 0;

            foreach ($invoiceData['anticipos'] as $anticipo) {
                $prepayment = new \Greenter\Model\Sale\Prepayment();
                $prepayment->setTipoDocRel($anticipo['tipo_doc_rel'] ?? '02')
                           ->setNroDocRel($anticipo['nro_doc_rel'] ?? '')
                           ->setTotal($anticipo['total'] ?? 0);
                $anticipos[] = $prepayment;
    ...
    public function createNote(array $noteData): GreenterNote
    {
        ...
        if (isset($noteData['forma_pago_tipo']) && $noteData['forma_pago_tipo'] === 'Credito') {
            // Para código "13" (ajuste de plan de pagos), el monto en FormaPagoCredito
            // debe ser la suma de las cuotas, no mto_imp_venta (que es 0)
            $montoPago = $noteData['mto_imp_venta'];
            if (isset($noteData['cod_motivo']) && $noteData['cod_motivo'] === '13'
                && isset($noteData['forma_pago_cuotas']) && !empty($noteData['forma_pago_cuotas'])) {
                $montoPago = array_sum(array_column($noteData['forma_pago_cuotas'], 'monto'));
            }

            $note->setFormaPago(new FormaPagoCredito($montoPago));
    ...
    protected function createSaleDetails(array $detalles): array
    {
        ...
        foreach ($detalles as $detalle) {
            ...
            // ISC (opcional) - Requiere tip_sis_isc, porcentaje, base y monto
            if (isset($detalle['tip_sis_isc']) &&
                isset($detalle['porcentaje_isc']) &&
                $detalle['porcentaje_isc'] > 0 &&
                isset($detalle['isc']) &&
                $detalle['isc'] > 0) {

                $item->setTipSisIsc($detalle['tip_sis_isc'])
                     ->setMtoBaseIsc($detalle['mto_base_isc'] ?? $detalle['mto_valor_venta'])
                     ->setPorcentajeIsc($detalle['porcentaje_isc'])
                     ->setIsc($detalle['isc']);
            }
            ...
            // Descuentos por línea (opcional)
            if (isset($detalle['descuentos']) && is_array($detalle['descuentos']) && count($detalle['descuentos']) > 0) {
                $descuentos = [];
                foreach ($detalle['descuentos'] as $desc) {
                    $descuento = new \Greenter\Model\Sale\Charge();
                    $descuento->setCodTipo($desc['cod_tipo'] ?? '00')
                              ->setMontoBase($desc['monto_base'] ?? 0)
                              ->setFactor($desc['factor'] ?? 0)
                              ->setMonto($desc['monto'] ?? 0);
                    $descuentos[] = $descuento;
                }
                $item->setDescuentos($descuentos);
            }
